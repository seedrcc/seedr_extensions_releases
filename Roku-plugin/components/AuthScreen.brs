    ...
    if press then
        ...
        if key = "OK" or key = "select" then
            ...
            if m.signInButton.hasFocus() then
                ...
                ' Trigger auth with 2 second delay
                print "[AuthScreen] Triggering authentication with 2 second delay"
                m.authDelayTimer = CreateObject("roSGNode", "Timer")
                m.authDelayTimer.duration = 2.0
