...
sub showSeedrHomeScene()
    print "[MainScene] Showing SeedrHomeScene..."
    m.authScreen.visible = false
    m.seedrHomeScene.visible = true
    m.videoPlayer.visible = false
    print "[MainScene] SeedrHomeScene visibility set to: "; m.seedrHomeScene.visible
end sub
...
sub onErrorOkPressed()
    ...
    if m.seedrHomeScene.visible then
        m.seedrHomeScene.findNode("button_group_1").setFocus(true)
