    ...
    if selectedIndex <> invalid and selectedIndex.count() >= 2
        ...
        if item <> invalid
            ...
            if item.fileType = "video" or item.fileType = "audio"
                ...
                print "[FolderDetailsScreen] Viewing document file: "; item.title
                showDocumentFile(item)
...
sub showImageFile(fileItem as object)
    print "[FolderDetailsScreen] Showing image: "; fileItem.title
    ...
    if sceneFound and parentNode <> invalid then
        ...
        parentNode.showImageViewer = {
            imageData: imageData,
            source: "FolderDetailsScreen"
        }
        print "[FolderDetailsScreen] Signaled parent to show image viewer"
    else
        print "[FolderDetailsScreen] ERROR: Could not find parent scene for image viewer"
    end if
end sub

' Show document file
sub showDocumentFile(fileItem as object)
    print "[FolderDetailsScreen] Showing document: "; fileItem.title
    ...
    if sceneFound and parentNode <> invalid then
        ...
        print "[FolderDetailsScreen] Signaled parent to show document viewer"
    else
        print "[FolderDetailsScreen] ERROR: Could not find parent scene for document viewer"
    end if
end sub

' Show file information (for unknown file types)
sub showFileInfo(fileItem as object)
    print "[FolderDetailsScreen] File info for: "; fileItem.title

    ' For now, just print to console - could be enhanced with a dialog
    if fileItem.fileData <> invalid then
        print "[FolderDetailsScreen] File size: "; fileItem.fileData.size
        print "[FolderDetailsScreen] File type: "; fileItem.fileType
    end if

    ' Could show a simple info dialog here in the future
end sub
