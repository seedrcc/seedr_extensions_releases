...
sub onPlayVideo()
    ...
    if playVideoData <> invalid then
        ...
        if playVideoData.folderData <> invalid and playVideoData.folderData.id <> invalid then
            ' From FolderDetailsScreen (has specific folder ID)
            m.audioPlaybackSource = "FolderDetailsScreen"
        else
            ' From root folder (SeedrHomeScene)
    ...
    if m.pendingVideoData <> invalid and m.pendingVideoData.folderData <> invalid then
        ...
        for each file in folderData.files
            if file <> invalid and file.title <> invalid then
                print "[HeroMainScene] Checking file: "; file.title; " - isAudio: "; isAudioFile(file.title)
                if isAudioFile(file.title) then
                    audioFiles.Push(file)
                    print "[HeroMainScene] Added to playlist: "; file.title
                end if
            else
                print "[HeroMainScene] Skipping invalid file or file without title"
            end if
        end for
        ...
        if m.seedrAudioPlayer <> invalid then
            ...
            print "[HeroMainScene] SUCCESS: Successfully sent playlist to audio player"
        else
