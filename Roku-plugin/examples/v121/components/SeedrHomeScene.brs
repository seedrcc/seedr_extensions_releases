...
sub createRowBorders()
    ...
    if m.row_list.content <> invalid then
        ' Calculate row positions for 3 rows - using full screen height
        ' Note: rowY, rowHeight, and rowSpacing variables are calculated but not used
        ' as borders were removed for cleaner UI design
        rowY = 120 ' Starting Y position of RowList (moved down to avoid overlap)
        rowHeight = 220 ' Height of each row to fill screen
        rowSpacing = 0 ' No spacing to maximize space usage
    ...
    for each file in files
        ...
        if posterUrl <> "" then
            ...
            if isAudioFile(file.name) then
                ...
                item.HDPosterUrl = "pkg:/images/icon_focus_hd.jpg" ' Could add image icon later
            else
...
sub onRowListFocused()
    ...
    if rowListFocusedItem <> invalid then
        setCategoryName(rowListFocusedItem)
    end if
end sub
...
function onKeyEvent(key as string, press as boolean) as boolean
    ...
    if key = "OK" and m.button_group_1.isInFocusChain() then
        ...
        if selectedButton = 0 then
            ...
            m.row_list.jumpToItem = selectedButton
    ...
    if press = true then
        if key = "up" and m.currentCategory = 0 then
            ...
            if m.video.hasFocus() then
                ...
                m.row_list.setFocus(true)
                handled = true
            else if m.top.currentFolderId = "" then
                ' On home screen, cycle through category buttons instead of exiting
...
sub setCategoryName(index as integer)
    ' Breadcrumb removed - no longer needed
    ' Note: index parameter is intentionally unused as breadcrumb functionality was removed
end sub
...
sub handleFileSelection(fileItem as object)
    ...
    if fileData <> invalid then
        ...
        if isVideoFile(fileItem.title) then
            ...
            print "[SeedrHomeScene] File is DOCUMENT (filename) - showing info"
            showFileInfo(fileItem)
        else
...
sub setupFolderFilesForAudioPlayer(currentFileItem as object)
    ...
    if m.currentFolderContents <> invalid then
        ...
        if m.currentFolderContents.files <> invalid then
            ...
            for each file in m.currentFolderContents.files
                if file <> invalid and file.title <> invalid then
                    print "[SeedrHomeScene] Checking file: "; file.title; " - isAudio: "; isAudioFile(file.title)
                    if isAudioFile(file.title) then
                        audioFiles.Push(file)
                        print "[SeedrHomeScene] Added to playlist: "; file.title
                    end if
                else
                    print "[SeedrHomeScene] Skipping invalid file or file without title"
                end if
            end for
        else
...
sub showImageFile(fileItem as object)
    ...
    ' For now, just show file info - could be enhanced to show image viewer
    showFileInfo(fileItem)
end sub
            ...
            if isVideoFile(file.name) or isImageFile(file.name) or isAudioFile(file.name)
                ...
                if posterUrl <> ""
                    ...
                    if isVideoFile(file.name)
                        fileItem.hdPosterUrl = "pkg:/images/icon_focus_hd.jpg"
