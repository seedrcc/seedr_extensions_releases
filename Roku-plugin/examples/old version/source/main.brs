...
sub Main()
    ...
    if screen <> invalid then
        ...
        handleAuthentication(screen, region, font)
...
sub handleAuthentication(screen as object, region as object, font as object)
    ...
    if hasValidToken() then
        ...
        showSeedrRootFolders(screen, region, font)
        return
    end if
...
sub showSeedrRootFolders(screen as object, region as object, font as object)
    ...
    showFolderBrowserWithNavigation(screen, region, font, rootContents, "Home", credentials.accessToken)
end sub
...
sub showFolderBrowserWithNavigation(screen as object, region as object, font as object, contents as object, currentPath as string, accessToken as string)
    ...
    while true
        ...
        drawFolderPage(screen, region, font, allItems, selectedIndex, currentPath, currentPage, itemsPerPage)
                ...
                if key = 2 then ' Up arrow
                    ...
                    if selectedItem.itemType = "folder" then
                        ...
                        if folderContents <> invalid then
                            ...
                            showFolderBrowserWithNavigation(screen, region, font, folderContents, newPath, accessToken)
                        else
                        ...
                        if isVideoFile(fileName) then
                            ...
                            playAudioFile(screen, region, font, selectedItem, accessToken)
                        else
...
sub drawFolderPage(screen as object, region as object, font as object, allItems as object, selectedIndex as integer, currentPath as string, currentPage as integer, itemsPerPage as integer)
...
sub drawVideoPreview(region as object, item as object, itemName as string, panelX as integer, panelY as integer, panelW as integer, panelH as integer, titleFont as object, itemFont as object, detailFont as object, infoFont as object, primaryText as integer, secondaryText as integer, accentText as integer)
    ...
    if thumbnailUrl <> "" then
        ...
        if titleFont <> invalid then
            region.drawText("🎬", posterX + 80, posterY + 120, videoColor, titleFont)
        end if
        if infoFont <> invalid then
