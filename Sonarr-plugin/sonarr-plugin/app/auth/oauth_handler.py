...
class OAuthHandler:
    def __init__(self, config: SeedrConfig):
        ...
        self.token_file = os.path.join(str(get_config_dir()), "seedr_token.json")
    ...
    def start_device_flow(self) -> Dict[str, str]:
        ...
        print(f"[AUTH] Starting device flow with client_id: {self.config.client_id[:20]}...")
        print(f"[AUTH] API endpoint: {self.config.api_base_url}/api/v0.1/p/oauth/device/code")
        ...
        try:
            response = requests.post(
                ...
                data={
                    "client_id": self.config.client_id,
                    "scope": "files.read profile files.write tasks.write tasks.read account.read media.read"
                },
                timeout=30
            )
            ...
            error_detail = f"HTTP {e.response.status_code}: {error_text[:500]}"
            print(f"[AUTH ERROR] {error_detail}")
            raise ValueError(f"Failed to start OAuth device flow: {error_detail}")
        except Exception as e:
            print(f"[AUTH ERROR] Unexpected error: {str(e)}")
            raise ValueError(f"Failed to start OAuth device flow: {str(e)}")
