...
from .utils.paths import (
    ensure_all_directories, get_log_file, get_config_dir, 
    get_templates_dir, get_static_dir, get_app_data_dir,
    get_torrents_dir, get_completed_dir, get_processed_dir, get_error_dir
)
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(str(get_log_file()))
    ]
)
...
class DownloadRequest(BaseModel):
    ...
    title: Optional[str] = Field(None, description="Title of the download (optional, auto-detected if not provided)")
    ...
    notify_sonarr: Optional[bool] = Field(False, description="Notify Sonarr after download completes")
...
async def auth_login(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if "400" in error_msg:
            raise HTTPException(
                status_code=400, 
                detail=f"Seedr authentication failed. This may be due to an invalid client ID or API changes. Error: {error_msg}"
            )
        raise HTTPException(status_code=500, detail=f"Failed to start authentication: {error_msg}")
    except Exception as e:
...
async def start_watcher(
    ...
    config_file = os.path.join(str(get_config_dir()), "watcher_config.json")
    settings = {}
    ...
    if not torrent_dir:
        torrent_dir = settings.get("torrent_dir")
        if not torrent_dir:
            torrent_dir = str(get_torrents_dir())
    
    if not download_dir:
        download_dir = settings.get("download_dir")
        if not download_dir:
            download_dir = str(get_completed_dir())
    ...
    watcher_config_path = str(get_config_dir())
...
async def get_version():
    ...
    Returns current version, build date, and description.
    Useful for checking if updates are needed.
    """
    return {
        "version": __version__,
...
async def check_update():
    ...
    Returns:
        JSON with update availability and version information
    """
    try:
...
async def install_update(background_tasks: BackgroundTasks, request: Request):
    """
    Download and install an update.
    
    This will download the update, prepare the updater script,
    and schedule the application to restart.
    ...
    try:
        ...
        if not version or not download_url:
            raise HTTPException(
                status_code=400,
                detail="Missing version or download_url in request body"
            )
        
        update_service = UpdateService(current_version=__version__)
        
        # Download and prepare update
        result = update_service.download_and_install_update(
            download_url=download_url,
            version=version
        )
        
        if result["success"]:
            # Schedule the update restart in background
            from pathlib import Path
            updater_script = Path(result["updater_script"])
            
            # Trigger restart after a short delay
            def trigger_restart():
                import time
                time.sleep(3)  # Give time for response to be sent
                update_service.trigger_update_restart(updater_script)
...
def start_background_watcher():
    ...
    if os.path.exists(config_file):
        ...
        try:
            ...
            default_torrent_dir = str(get_torrents_dir())
            default_download_dir = str(get_completed_dir())
